...
class MessagesController extends Controller
{
  ...
  public function send(Request $request)
  {
    ...
    $path = config('path.messages');
     ...
     if($request->get('from_user')){
      $from_user = User::findOrFail($request->get('from_user'));
     }
     // Currency Position
     if ($settings->currency_position == 'right') {
      ...
      // Setup the validator
        ...
        if($request->get('from_user') && $from_user){
          $conversation = Conversations::where('user_1', $from_user->id)
  				->where('user_2', $user->id)
  				->orWhere('user_1', $user->id)
  				->where('user_2', $from_user->id)->first();  
        }else{
        ...
        if (! isset($conversation)) {
          ...
          if($request->get('from_user') && $from_user){
            $newConversation->user_1 = $from_user->id;  
          }else{
        ...
        if ($request->hasFile('zip')) {
          ...
          if($request->get('from_user') && $from_user){
            $file            = strtolower($from_user->id.time().Str::random(20).'.'.$extension);
          }else{
        ...
        if ($settings->push_notification_status && $getPushNotificationDevices && $diffInMinutes > 10 && $diffInMinutes < 1000) {
          ...
          if($request->get('from_user') && $from_user){
            $messagePush = __('general.new_msg_from').' @'.$from_user->username;
            $linkDestination = url('messages', $from_user->id());
          }else{
