...
class SubscriptionsController extends Controller
{
  ...
  public function subscriptionFree()
  {
    ...
    $sql->free = 'yes';
    $sql->save();
    $this->sendWelcomeMessage($creator->id);
  ...
  /**
   ...
   protected function sendWallet()
   {
	 ...
	 * @return Response
	 */
   public static function sendWelcomeMessage($userId)
   {
     // Find user
     $curr_user = auth()->user()->id;
     $welcomeMessage = AutomatedMessage::where("user_id", $userId)->where("type","welcome-message")->orderBy("id", "DESC")->first();
     if($welcomeMessage) {
        $mediaAutomatedMessage = MediaAutomatedMessage::where("welcome_message_id", $welcomeMessage->id)->get();
        $wel_msg = $welcomeMessage->message;
      
          $files = [];
          foreach ($mediaAutomatedMessage  as $key => $value) {
              $files[] = ["file" => $value->file];
          }

          $fileJson = json_encode($files);
          $data = ['id_user' => $curr_user,'from_user'=>$userId, 'message' => $wel_msg,"price"=>$welcomeMessage->price,"fileuploader-list-media"=>$fileJson,"_token"=>request()->header('X-Csrf-Token')];
          $request = Request::create('/message/send', 'POST', $data);
          $result = (new MessagesController)->send($request);
      }
      //freebie
      $freebie = AutomatedMessage::where("user_id", $userId)->where("type","freebies")->orderBy("id", "DESC")->first();
      if($freebie) {

        $mediaAutomatedMessage = MediaAutomatedMessage::where("welcome_message_id", $freebie->id)->get();
        $wel_msg = $freebie->message;
        ...
        $fileJson = json_encode($files);
        $data = ['id_user' => $curr_user,'from_user'=>$userId, 'message' => $wel_msg,"price"=>"","fileuploader-list-media"=>$fileJson,"_token"=>request()->header('X-Csrf-Token')];
        $request = Request::create('/message/send', 'POST', $data);
        $result = (new MessagesController)->send($request);
      }
   }
