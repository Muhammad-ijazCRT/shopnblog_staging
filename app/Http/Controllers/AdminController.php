...
class AdminController extends Controller
{
		...
		$datalastSales = implode(',', $lastSubscriptions);
		 	...
		 	return view('admin.dashboard', [
				...
				'total_raised_funds' => $total_raised_funds,
				 'total_raised_funds_moderator' => $total_raised_funds_moderator,
	...
	public function getFileVerification($filename)
  {
	...
	public function moderatorSettings(Request $request)
	{
    	return view('admin.moderator-setting')->with([
            'fee_moderator_admin_commission' => $this->settings->fee_moderator_admin_commission,
            'stripe_moderator_account_id' => $this->settings->stripe_moderator_account_id,
            'moderator_payment_enable' => $this->settings->moderator_payment_enable,
			]);

	}//<--- End Method

	public function storeModeratorSettings(Request $request)
	{
        if (!auth()->user()->role == 'admin') {
			return back()->withErrorMessage(trans('general.give_access_error'));
		}
        $sql = AdminSettings::first();
        $sql->fee_moderator_admin_commission = $request->fee_moderator_admin_commission;
        $sql->stripe_moderator_account_id = $request->stripe_moderator_account_id;
        $sql->moderator_payment_enable = $request->moderator_payment_enable;
        $sql->save();
