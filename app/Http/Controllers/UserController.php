...
class UserController extends Controller
{
  ...
  public function postDetail($slug, $id)
  {
   ...
   public function notifications()
    ...
    {
      $welcomeMessage = AutomatedMessage::where("type", "welcome-message")
                                          ->where("user_id", auth()->user()->id)
                                          ->orderBy("id", "desc")->first();

      return view('users.welcome_message', compact('welcomeMessage'));
    }

    public function jsonWelcomeMsgMediaFile($welcomeMessageId) 
    {
      $mediaFiles = MediaAutomatedMessage::where("welcome_message_id", $welcomeMessageId)->orderBy("id", "desc")->get();
      return $mediaFiles;
    }

    public function updateWelcomeMessage()
    {
       $type = request()->type ? request()->type : "welcome-message";
       $checkWelcomMsg = AutomatedMessage::where("user_id", auth()->user()->id)
                                          ->where("type", $type)
                                          ->orderBy('id', 'DESC')
                                          ->first();
...
//            unset($body["message"]);
          ...
          $update = AutomatedMessage::where("id", $checkWelcomMsg->id)
                                    ->update($body);
          $mediaFiles = MediaAutomatedMessage::where("welcome_message_id", $checkWelcomMsg->id)->get();
          $mediaFileNameArr = [];
          if(request()->media) {
            foreach (request()->media as $key => $value) {
              $isExist = false;
              $mediaFileNameArr[] = $value["filename"];
              foreach ($mediaFiles as $key => $media) {
                if($media->file == $value["filename"]) {
                  $isExist = true;
                }
              }
              if($isExist == false) {
                  /** STORE MEDIA FILE */
                  $media = new MediaAutomatedMessage();
                  $media->user_id = auth()->user()->id;
                  $media->welcome_message_id = $checkWelcomMsg->id;
                  $media->type = $value["type"];
                  $media->ext = $value["ext"];
                  $media->size = $value["size"];
                  $media->file = $value["filename"];
                  // $media->price = $value["price"];
                  $media->save();
              } else {
                /** UPDATE EXISTING MEDIA FILE  */
                $update = MediaAutomatedMessage::where("welcome_message_id", $checkWelcomMsg->id)
                                          ->where("file", $value["filename"])
                                          ->where("user_id", auth()->user()->id)
                                          ->update([
                                            "file" => $value["filename"],
                                            // "price" => $value["price"],
          ...
          foreach ($mediaFiles as $key => $media) {
            if(!in_array($media->file, $mediaFileNameArr)) {
              $delete = MediaAutomatedMessage::where("id", $media->id)->delete();
            }
          }
       } else {
          /** Create Welcome Message */
          $automatedMsg = new AutomatedMessage();
          $automatedMsg->user_id = auth()->user()->id;
          $automatedMsg->type = $type;
          if(!request()->type) {
            $automatedMsg->price = request()->price;
            $automatedMsg->message = request()->welcome_message;
          }
          if($automatedMsg->save()) {
            foreach (request()->media as $key => $value) {
              $media = new MediaAutomatedMessage();
              $media->user_id = auth()->user()->id;
              $media->welcome_message_id = $automatedMsg->id;
              $media->type = $value["type"];
              $media->ext = $value["ext"];
              $media->size = $value["size"];
              $media->file = $value["filename"];
    ...
    {
      $freebies = AutomatedMessage::where("type", "freebies")
                                          ->where("user_id", auth()->user()->id)
                                          ->orderBy("id", "desc")->first();
                                          
      return view('users.freebies', compact('freebies'));
    }
  ...
  protected function createPlanPaystack()
  {
   ...
   $path = config('path.cover');
    ...
    public function mySales()
    {
      ...
      $sales->when(request('sort') == 'intransit', function($q) {
        $q->where('delivery_status', 'inst');
      });
  ...
  {
    $purchase = Purchases::where('id', $request->id)->firstOrFail();
    $purchase->tracking_no = $request->tracking_no;
    $purchase->carrier = $request->carrier;
    $purchase->save();

    $buyer = Purchases::find($request->id)->user_id;
    $seller = Products::find(Purchases::find($request->id)->products_id)->user_id;

    $date = Purchases::where('id', $request->id)->firstOrFail()->created_at;
