...
// Route::get('/new_success', function (Request $request) {
...
//         $purchase->cu_cuntry = $product_data["cu_cuntry"];
//         $purchase->name = $product_data["name"];
//         $purchase->email = $product_data["email"];
//         $purchase->state = $product_data["state"];
//         $purchase->address = $product_data["address"];
//         $purchase->city = $product_data["city"];
//         $purchase->zip = $product_data["zip"];
//         $purchase->phone = $product_data["phone"];
//         $purchase->save();
...
//           return url('shop/product', $item->id);
//         } else {
// 			return  Helper::userWallet();
// 		}
...
// Route::get('/new_success', function (Request $request) {})->name('new_success');

Route::get('/new_success', [ProductsController::class, 'succ'])->name('new_success');
...
Route::get('/paypal/success', function (Request $request) {
	...
	// $deposit_user = request()->query('deposit_user');
	// $total_deposit = request()->query('total_deposit');
	// $deposit_commission = request()->query('deposit_commission');
	
	// $deposit_commission = new DepositCommission;
	// $deposit_commission->payment_method = $payment_method;
	// $deposit_commission->deposit_user = $deposit_user;
	// $deposit_commission->total_deposit = $total_deposit;
	// $deposit_commission->deposit_commission = $deposit_commission;
...
Route::get('verify/account/{confirmation_code}', [HomeController::class, 'getVerifyAccount'])->where('confirmation_code','[A-Za-z0-9]+');
 ...
 Route::get('buy/subscription/success/{user}/{data?}', function($user, $subscription_amount) {
	$calculated_admin_commission = $subscription_amount * 0.15;

	$admin_commission = new Admin_earning;
	$admin_commission->amount = $calculated_admin_commission;
	$admin_commission->save();

	$userId = User::where('username',$user)->first();
	SubscriptionsController::sendWelcomeMessage($userId->id);
 ...
 Route::group(['middleware' => 'auth'], function() {
     ...
     Route::get('automated/{id}', [SubscriptionsController::class, 'automatedSendMessage']);
    ...
    //My subscription welcome message
	Route::get('my/subscriptions/welcomeMessage',[UserController::class, 'welcomeMessage']);
	Route::get('my/subscriptions/welcomeMessage/media-files/{id}',[UserController::class, 'jsonWelcomeMsgMediaFile']);
	Route::post('my/subscriptions/welcomeMessage/update',[UserController::class, 'updateWelcomeMessage']);

	// //My subscription Freebie message
	Route::get('my/subscriptions/freebies',[UserController::class, 'freebies']);
    ...
    Route::post('get_spacial', [ProductsController::class, 'get_spacial'])->name('get_spacial');
	// Stripe Connect
	...
	Route::post('my/tracking',[UserController::class, 'tracking']);
...
Route::group(['middleware' => 'private.content'], function() {
	...
	// Creators
	Route::get('vendors/{type?}',[HomeController::class, 'creators']);
 ...
 Route::group(['middleware' => 'role'], function() {
	...
	// Moderator Earnings
	Route::get('panel/admin/moderator-admin','AdminController@moderatorSettings')->name('moderator');
	Route::post('panel/admin/moderator-admin', 'AdminController@storeModeratorSettings');

    // Moderator Settings Of Stripe
    Route::get('panel/admin/moderator-admin-stripe', 'ModeratorAdminController@paymentsGateways')->name('moderator-stripe');
    Route::post('panel/admin/moderator-admin-stripe', 'ModeratorAdminController@savePaymentsGateways');
	// Stripe Connect
	Route::get('panel/admin/moderator/stripe/connect', 'ModeratorAdminController@redirectToStripe')->name('moderator.redirect.stripe');
	Route::get('panel/admin/moderator/stripe/connect/view', 'ModeratorAdminController@redirectToStripeView')->name('moderator.redirect.stripe_view');
	Route::get('panel/admin/moderator/connect/{token}/{stripe_connect_id}', 'ModeratorAdminController@saveStripeAccount')->name('moderator.save.stripe');
