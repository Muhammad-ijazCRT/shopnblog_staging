    ...
    @endif
      ...
      @else
        ...
        @if ($product->shipping_fee <> 0.00)
          <div class="w-100 d-block mt-4">
            <i class="bi-truck mr-2"></i> {{ __('general.shipping_feesss') }} - {{ Helper::amountFormatDecimal($product->shipping_fee) }} <small>{{ $settings->currency_code }}</small>
...
$("#cu_free").on('change',function(){
 ...
 $('#cu_cun_buy').on('change', function() {
     ...
     $("#spacia_shiping").hide();
       ...
       $.ajax({
           type: "POST",
           url:"{{route('get_spacial')}}",
           dataType: "json",
            data: {name:name, id:id,_token: "{{ csrf_token() }}"},
           success: function( msg ) {
               if(msg.success==true){
               $("#spacia_shiping").show();
               $(".totalsp").text(msg.total);
               $("#shipingfees").text(msg.ship);
               $("#spone").hide();
               $("#sptwo").show();
               $("#shiping_disa").hide();
               }else if(msg.success==2){
                   alert("{{__('general.disabe_cu')}}");
                $(".totalsp").text(msg.total);
               }else{
                $(".totalsp").text(msg.total);
                 $("#spone").hide();
...
<script type="text/javascript">
   ...
   var free_shiping=$("#cu_free option:selected").text();
        ++is;
        if(country !='' && $("input").hasClass(countryts)===false && free_shiping!=countryts){
        $("#ff").append('<tr><td>'+countryts+'</td><td><input type="hidden" class="cuc form-control '+countryts+'" name="addmores['+is+']" value='+country+' placeholder="Enter Fees" class="form-control" /><button aria-hidden="true" class="remove-tr">&times;</button></td></tr>');
