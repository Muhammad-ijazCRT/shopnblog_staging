<header>
	<nav class="navbar navbar-expand-lg navbar-inverse fixed-top p-nav bg-whites bg-white">
		<div class="container-fluid d-flex position-relative">
			...
			@guest
				<button style="background-color: black;" class="navbar-toggler @if(auth()->guest() && request()->path() == '/') text-white @endif" type="button" data-toggle="collapse" data-target="#navbarCollapse" aria-controls="navbarCollapse" aria-expanded="false" aria-label="Toggle navigation">
			...
			@if (auth()->guest() && $settings->who_can_see_content == 'all' || auth()->check())
				<ul class="navbar-nav mr-auto">
					<form class="form-inline my-lg-0 position-relative" method="get" action="{{url('vendors')}}">
						<input id="searchCreatorNavbar" class="form-control search-bar @if(auth()->guest() && request()->path() == '/') border-0 @endif" type="text" required name="q" autocomplete="off" minlength="3" placeholder="Find a vendor" aria-label="Search">
					...
					@guest
						<li class="nav-item hoverable">
							<a class="nav-link text-dark" href="{{url('vendors')}}">{{trans('general.explore')}}</a>
						</li>

						@if ($settings->shop)
						<li class="nav-item hoverable">
							<a class="nav-link text-dark" href="{{url('shop')}}">{{trans('general.shop')}}</a>
						</li>
					@endif
					@endguest
			...
			@else
				...
				@if (auth()->user()->role == 'admin' || auth()->user()->role == 'moderator_admin')
					...
					<li class="nav-item dropdown d-lg-none">
						<a href="{{url('my/purchases')}}" class="nav-link px-2 link-menu-mobile py-1">
							<div>
								...
								<span class="d-lg-none">{{ trans('general.purchaseds') }}</span>
							</div>
						</a>
					</li>
					
					<li class="nav-item dropdown d-lg-none border-bottom">
						<a href="{{url('my/purchased/items')}}" class="nav-link px-2 link-menu-mobile py-1">
							<div>
								<i class="bi bi-cart-check mr-2"></i>
								<span class="d-lg-none">{{ trans('general.purchased_sh') }}</span>
							</div>
						</a>
					</li>
			...
			@endif
				...
				<li class="nav-item dropdown d-lg-block d-none">
					<a class="nav-link" href="#" id="nav-inner-success_dropdown_1" role="button" data-toggle="dropdown">
						<img src="{{Helper::getFile(config('path.avatar').auth()->user()->avatar)}}" alt="User" class="rounded-circle rounded-circles avatarUser mr-1" width="28" height="28">
