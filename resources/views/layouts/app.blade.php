...
@if (auth()->guest() && request()->path() == '/' && $settings->home_style == 0
  ...
  @if(!auth()->guest() && auth()->user()->dark_mode!='on')<style>.bg-whites {background-color: #e3e3e3!important;}.p-nav a.nav-link:not(.btn) {color: #8a96a3;}.rounded-circles {border: 1px solid;}</style>@endif
