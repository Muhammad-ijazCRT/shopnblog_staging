    ...
    <main>
      ...
      <div class="offcanvas-body px-0 scrollbar">
          <ul class="nav nav-pills flex-column mb-sm-auto mb-0 align-items-start list-sidebar" id="menu">
           ...
           @if (auth()->user()->hasPermission('moderator_earnings'))
              <li class="nav-item @if( Request::is('panel/admin/moderator-admin') || Request::is('panel/admin/moderator-admin-stripe') ) active @endif" @if(Request::is('panel/admin/moderator-admin') || Request::is('panel/admin/moderator-admin-stripe') ) aria-expanded="true" @endif>
                  <a href="#moderator_admin" data-bs-toggle="collapse" class="nav-link text-truncate dropdown-toggle @if (request()->is('panel/admin/settings') ||request()->is('panel/admin/settings/limits')) active @endif" @if (request()->is('panel/admin/settings') ||request()->is('panel/admin/settings/limits')) aria-expanded="true" @endif>
                      <i class="bi-gear me-2"></i> {{ trans('admin.moderator_admin') }}
                  </a>
              </li><!-- /end list -->
            @endif

              <div class="collapse w-100 @if( Request::is('panel/admin/moderator-admin') || Request::is('panel/admin/moderator-admin-stripe') ) show @endif ps-3" id="moderator_admin">
               
                
                
                <li >
                        <a class="nav-link text-truncate w-100 @if (request()->is('panel/admin/moderator-admin')) text-white @endif" href="{{ url('panel/admin/moderator-admin') }}"><i class="bi-chevron-right fs-7 me-1"></i> {{ trans('admin.earning_settings') }}</a>
                    </li>
                    <li >
                        <a class="nav-link text-truncate w-100 @if (request()->is('panel/admin/moderator-admin-stripe')) text-white @endif" href="{{ url('panel/admin/moderator-admin-stripe') }}"><i class="bi-chevron-right fs-7 me-1"></i> {{ trans('admin.moderator_admin_stripe') }}</a>
                    </li>
